    ...
    return [
        {
            ...
            "inputSchema": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "Function name or description to search for"}
                },
                "required": ["query"]
            }
        },
        {
            ...
            "inputSchema": {
                "type": "object",
                "properties": {
                    "functionName": {"type": "string", "description": "Exact function name from __all__ exports"},
                    "includeExamples": {"type": "boolean", "description": "Include usage examples from tests"}
                },
...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if name == "resolve-function-id":
        ...
        for func_name, func in functions.items():
            ...
            if query in func_name.lower() or query in doc:
                results.append({
                    ...
                    "parameters": len(inspect.signature(func).parameters) if hasattr(func, "__signature__") or inspect.isfunction(func) else 0
                })
        ...
        return [{"type": "text", "text": f"Function: {func_name}{sig}\nCategory: {category}\n\n{doc}"}]
